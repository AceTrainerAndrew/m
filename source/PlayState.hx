...
class PlayState extends MusicBeatState
{
	...
	public static var Emerald:Bool = false;
	...
	private var ShadowAceDuo:Character;
	private var Ace:Character;
	...
	public var dialogue:Array<String> = ['dad:blah blah blah', 'bf:bababooey B)'];

	var halloweenBG:FlxSprite;
	var aceBG:FlxSprite;
	var aceBGEVIL:FlxSprite;
	...
	override public function create()
	{
		...
		islwb = (SONG.song.toLowerCase() == 'long-wired-brawl' || SONG.song.toLowerCase() == 'waterstream' || SONG.song.toLowerCase() == 'agoti-but-ace' || SONG.song.toLowerCase() == 'scythe' || SONG.song.toLowerCase() == 'corrupt' || SONG.song.toLowerCase() == 'waterstream'); // my ass
		minusHealth = false; 
		...
		switch (songLowercase)
		{
			...
			case 'siu':
				dialogue = CoolUtil.coolTextFile(Paths.txt('siu/Dialogue'));
			case 'roses':
			...
			case 'scythe':
				dialogue = CoolUtil.coolTextFile(Paths.txt('scythe/scytheDialogue'));
			case 'outage':
				dialogue = CoolUtil.coolTextFile(Paths.txt('outage/outageDialogue'));
			case 'waterstream':
		...
		if (SONG.stage == null) {
			switch(storyWeek)
			{
				case 2: if (songLowercase == 'monster') {stageCheck = 'scary';} else {stageCheck = 'halloween';}
				case 3: stageCheck = 'philly';
				case 4: stageCheck = 'limo';
				case 5: if (songLowercase == 'winter-horrorland') {stageCheck = 'mallEvil';} else {stageCheck = 'mall';}
				case 6: if (songLowercase == 'scythe' || songLowercase == 'waterstream') {stageCheck = 'AcesHouseEvil';} else {stageCheck = 'AcesHouse';}
		...
		switch(stageCheck)
		{
			...
			case 'scary': 
			{
				...
				var hallowTex = Paths.getSparrowAtlas('scary_bg','week2');
				...
				for (i in 0...2)
				{
					var daFire:SiniFire = new SiniFire(halloweenBG.x + (720 + (((95 * 10) / 2) * i)), halloweenBG.y + 180, true, false, 30, i * 10, 84);
				...
				var fire1:SiniFire = new SiniFire(halloweenBG.x + (-100), halloweenBG.y + 889, true, false, 30);
				...
				var fire2:SiniFire = new SiniFire((fire1.x + fire1.width) - 80, halloweenBG.y + 889, true, false, 30);
				fire2.antialiasing = true;
				fire2.scrollFactor.set(0.9, 0.9);
				//fire2.scale.set(2.5, 1);
				fire2.y -= fire2.height * 1;
			
				var fire3:SiniFire = new SiniFire((fire2.x + fire2.width) - 30, halloweenBG.y + 889, true, false, 30);
				fire3.antialiasing = true;
				fire3.scrollFactor.set(0.9, 0.9);
				//fire3.scale.set(2.5, 1);
				fire3.y -= fire3.height * 1;
				
				var fire4:SiniFire = new SiniFire((fire3.x + fire3.width) - 10, halloweenBG.y + 889, true, false, 30);
			...
			case 'spongeballer':
			{
					curStage = 'spongeballer';
					defaultCamZoom = 1;

					var skyBG:FlxSprite = new FlxSprite(-350, 0).loadGraphic(Paths.image('stageSPONGEBALLER/stageSPONGEBALLER','shared'));
					skyBG.scrollFactor.set(1, 1);
					skyBG.scale.set(1, 1);
					add(skyBG);

					var Dancers:FlxSprite = new FlxSprite(-350, -50).loadGraphic(Paths.image('stageSPONGEBALLER/BGDancers','shared'));
					Dancers.scrollFactor.set(1, 1);
					add(Dancers);
...
/*					curStage = 'AcesHouse';
					...
					var bgSchool:FlxSprite = new FlxSprite(repositionShit, 0).loadGraphic(Paths.image('weeb/aceBG','week6'));
					bgSchool.scrollFactor.set(0.6, 0.90);
					add(bgSchool);

					var bgStreet:FlxSprite = new FlxSprite(repositionShit).loadGraphic(Paths.image('weeb/aceBGFG','week6'));
					...
					if (songLowercase == 'blade-trap' || songLowercase == 'long-wired-brawl')
						{
							//bgGirls.getScared();
							
						//	BGLuna = new BackgroundLuna(-150, 400);
							BGLuna = new BackgroundLuna(-200, 0);
							
							BGLuna.setGraphicSize(Std.int(BGLuna.width * 0.6));
							add(BGLuna);
							BGLuna.dance();
					}
...
*/		
				...
				var hallowTex = Paths.getSparrowAtlas('weeb/AcesHouse','week6');

				aceBG = new FlxSprite(-350,0);
				aceBG.frames = hallowTex;
				aceBG.animation.addByPrefix('idle', 'HouseNORMAL');
				aceBG.animation.play('idle');
				aceBG.antialiasing = true;
				add(aceBG);
				
				aceBG.setGraphicSize(widShit);
				aceBG.updateHitbox();
...
*/
				...
				var hallowTex = Paths.getSparrowAtlas('weeb/AcesHouseEVIL','week6');

				aceBGEVIL = new FlxSprite(-350, 0);
				aceBGEVIL.frames = hallowTex;
				aceBGEVIL.animation.addByPrefix('idle', 'HouseEVIL');
				aceBGEVIL.animation.play('idle');
				aceBGEVIL.antialiasing = true;
				add(aceBGEVIL);
				
				aceBGEVIL.setGraphicSize(widShit);
				aceBGEVIL.updateHitbox();
			}
...
*/			
		...
		switch (CharacterState.selectedChar)
		{
		case 'aceman':
				boyfriend.y -= 50;
		case 'bf-cb':
...
if (songLowercase != 'conflict' && songLowercase != 'acelistic' && songLowercase != 'execution' && songLowercase != 'vampirerays')
	...
	//luna-duo		
	//	if (SONG.song == 'danger' && dad.curCharacter == 'luna-duo')
	//		add(ace);
		if (exDad) add(Ace);
		if (exDad2) add(Ace);
		if (exDad3) add(Ace);
		...
		switch (dad.curCharacter)
		{
		   ...
		   case 'senpaihd':
		  {
		    healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			  'health', 0, 2);
		    healthBar.scrollFactor.set();
		    healthBar.createFilledBar(0xFF000000, 0xFF31b0d1); // black
		  ...
		  case 'z11':
		  {
		    healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			  'health', 0, 2);
		    healthBar.scrollFactor.set();
		    healthBar.createFilledBar(0xFF000000, 0xFFffd700); // black & gold
		    // healthBar for acespritduo
		    add(healthBar);
		  }
		   case 'andrew':
		  ...
		  case 'spongeballer':
		  {
		    healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			  'health', 0, 2);
		    healthBar.scrollFactor.set();
		    healthBar.createFilledBar(FlxColor.GRAY, 0xFFffd700); // black
		    // healthBar for andrew
		    add(healthBar);
		  }
		  ...
		  case 'cb':
		  {
		    healthBar = new FlxBar(healthBarBG.x + 4, healthBarBG.y + 4, RIGHT_TO_LEFT, Std.int(healthBarBG.width - 8), Std.int(healthBarBG.height - 8), this,
			  'health', 0, 2);
		    healthBar.scrollFactor.set();
		    healthBar.createFilledBar(0xFF2864af, 0xFF31b0d1); // CB's color
		    // healthBar for acespritduo
		    add(healthBar);
		  }
	...
	*/
	...
	//	iconP1 = new HealthIcon(CharacterState.selectedChar, true);
	...
	function schoolIntro(?dialogueBox:DialogueBox):Void
	{
		...
		if (StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'waterstream' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'waterstream' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'thorns' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'corrupt' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'blade-trap' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'long-wired-brawl' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'corrupt' || StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'scythe')
		{
			remove(black);
		...
		new FlxTimer().start(0.3, function(tmr:FlxTimer) 
		{
			...
			else
			{
				if (dialogueBox != null)
				{
					...
					if (StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() == 'bruhmomentlmaoxd')
					{
						add(senpaiEvil);
						senpaiEvil.alpha = 0;
...
*/						else if (StringTools.replace(PlayState.SONG.song, " ", "-").toLowerCase() != 'idk')
					{
						add(dialogueBox);
					}
				}
...
/*		if (FlxG.keys.justPressed.THREE)
		{
			boyfriend.playAnim('emerald', true);
		}
*/		
		switch (curStage)
		{
		...
		if (generatedMusic)
			{
				...
				{	
					...
					if (!daNote.mustPress && daNote.wasGoodHit)
					{
						...
						if (SONG.song.toLowerCase() == 'long-wired-brawl' || SONG.song.toLowerCase() == 'waterstream' || SONG.song == 'agoti-but-ace' || SONG.song.toLowerCase() == 'scythe' || SONG.song.toLowerCase() == 'corrupt')
						...
						if (!exDad && !exDad2 && !exDad3)
							...
							{							
								...
								{
									targ = Ace;
									if (daNote.dType == 1)  targ = dad;
									if (daNote.dType == 0) targ = Ace;
								...
								/*{
									if (daNote.dType == 2) both = true;
									if (daNote.noteData <= 3) targ = ShadowAceDuo;
									if (daNote.noteData == 4) both = true;
								//	if (daNote.dType == 2) targ = ShadowAceDuo || dad;
								}
								*/
				...
				/*					case 4:
								...
								if (both) ShadowAceDuo.playAnim('singUP' + altAnim, true);
	...
	*/
	...
	function emeraldhit():Void
	{
		var emeraldgif = new Emeraldspr(575, 285);

		health -= 0.5;
		FlxG.sound.play(Paths.sound('Glass-Break'));
		...
		new FlxTimer().start(1.15, function(tmr:FlxTimer)
			{
				FlxTween.tween(emeraldgif, {y: emeraldgif.y += 0, alpha: 0}, Conductor.crochet / 1000, {
						ease: FlxEase.cubeInOut,
						onComplete: function(twn:FlxTween)
						{
							emeraldgif.destroy();
...
/*		if (curSong.toLowerCase() == "blade-trap")
		{
            ...
            areyouprepared.screenCenter();
			
			switch (curStep)
				{
				case 230:
				add(areyouprepared);
				FlxTween.tween(FlxG.camera, {zoom: 1.65}, 1.0);
				new FlxTimer().start(3.5, function(tmr:FlxTimer)
				{
				});
				FlxTween.tween(areyouprepared, {alpha: 0},1, {
                ease: FlxEase.cubeInOut,
                onComplete: function(twn:FlxTween)
                {
                    areyouprepared.destroy();
                }
            });
			

					//FlxG.camera.scroll.x = -425;
